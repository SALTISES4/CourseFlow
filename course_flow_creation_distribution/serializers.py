...
class ParentStrategySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Strategy
        fields = ["id", "title", "description", "last_modified", "hash", "author"]
    ...
    def update(self, instance, validated_data):
        ...
        for outcomepreparation_data in self.initial_data.pop("outcomepreparation_set"):
            outcomepreparation_serializer = OutcomePreparationSerializer(
                OutcomePreparation.objects.get(id=outcomepreparation_data["id"]),
    ...
    def update(self, instance, validated_data):
        ...
        for weekcourse_data in self.initial_data.pop("weekcourse_set"):
            weekcourse_serializer = WeekCourseSerializer(
                WeekCourse.objects.get(id=weekcourse_data["id"]), data=weekcourse_data
            )
