    ...
    for act in OldCourse.objects.all():
        ...
        for week in act.weeks.all():
            ...
            for component in week.components.all():
                compobj = ContentType.objects.get(
                    model=component.content_type.model
                ).get_object_for_this_type(pk=component.object_id)
    ...
    for act in OldProgram.objects.all():
        ...
        for strat in act.outcomes.all():
            OutcomeWorkflow.objects.create(
                outcome=strat,
                workflow=newact,
                rank=OutcomeProgram.objects.get(
                    outcome=strat, program=act
                ).rank,
            )
        act.delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [("course_flow", "0008_auto_20200917_2034")]
    ...
    operations = [
        ...
        migrations.RunPython(
            switch_course_to_workflow, switch_course_to_noworkflow
        ),
        migrations.RunPython(
            switch_program_to_workflow, switch_program_to_noworkflow
        ),
