...
def get_queryset_by_created_month(queryset,month,year):
    return queryset.filter(created_on__month=month,created_on__year=year).count()

def filter_workflow_by_nodes(queryset):
    return queryset.annotate(num_nodes=Count('weeks__nodeweek')).filter(num_nodes__gte=3)

def filter_project_by_nodes(queryset):
    return queryset.annotate(num_nodes=Count('workflows__weekworkflow__week__nodeweek__node')).filter(num_nodes__gte=3)
        ...
        for month in range(first_month,last_month+1):
            rows+=[
                (
                    ...
                    get_queryset_by_created_month(activities_nodes,month,year),
...
def get_annoted_users_for_objects_by_month(type,month,year):
    ...
    return models.User.objects.filter(
        **{'authored_'+type+'__created_on__month':month,'authored_'+type+'__created_on__year':year}
    ).annotate(
...
def get_saltise_admin_users():
    ...
    for year in range(original_year,current_year+1):
        ...
        if year==current_year:
            last_month=current_month
        for month in range(first_month,last_month+1):
            annotated_activities = get_annoted_users_for_objects_by_month("activities",month,year)
            annotated_courses = get_annoted_users_for_objects_by_month("courses",month,year)
            annotated_programs = get_annoted_users_for_objects_by_month("programs",month,year)
            annotated_projects = get_annoted_users_for_objects_by_month("projects",month,year)
            annotated_nodes = get_annoted_users_for_objects_by_month("nodes",month,year)
            rows+=[
                (
                    "<tr>"
                    +"<td>{} {}</td>"
    ...
    rows+=[
        (
            ...
            get_users_for_object(models.User.objects.all().annotate(num_authored=Count("authored_projects"))),
            get_users_for_object(models.User.objects.all().annotate(num_authored=Count("authored_activities"))),
            get_users_for_object(models.User.objects.all().annotate(num_authored=Count("authored_courses"))),
            get_users_for_object(models.User.objects.all().annotate(num_authored=Count("authored_programs"))),
            get_users_for_object(models.User.objects.all().annotate(num_authored=Count("authored_nodes"))),
        )
    ]  
    table = "\n".join(rows)
    return format_html("<table>"+table+"</table>")
