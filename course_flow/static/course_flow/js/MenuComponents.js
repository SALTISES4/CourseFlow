...
export class MessageBox extends React.Component {
  render() {
    var menu;
    if (
      this.props.message_type == "linked_workflow_menu" ||
      this.props.message_type == "target_project_menu" ||
      this.props.message_type == "added_workflow_menu"
    )
      menu = (
        <WorkflowsMenu
    ...
    if (this.props.message_type == "project_edit_menu")
      menu = (
        <ProjectEditMenu
          type={this.props.message_type}
          data={this.props.message_data}
    ...
    if (this.props.message_type == "share_menu")
      menu = (
        <ShareMenu
          data={this.props.message_data}
          actionFunction={this.props.actionFunction}
    ...
    return (
      <div class="screen-barrier" onClick={evt => evt.stopPropagation()}>
        <div class="message-box">{menu}</div>
      </div>
    );
  }
}
...
export class WorkflowsMenu extends React.Component {
  constructor(props) {
    ...
    if (
      ...
      this.project_workflows = props.data.data_package.current_project.sections[0].objects.map(
        object => object.id
      );
  }
  ...
  render() {
    ...
    for (var prop in data_package) {
      tab_li.push(
        <li class="tab-header">
          <a href={"#tabs-" + i}>{data_package[prop].title}</a>
        </li>
      );
      tabs.push(
        <MenuTab
...
export class WorkflowForMenu extends React.Component {
    ...
    return (
      ...
      >
        ...
        <div class="workflow-publication">
          <img src={publish_icon} />
          <div>{publish_text}</div>
        </div>
      </div>
    );
  }
  ...
  clickAction() {
    if (this.props.selectAction) {
      ...
      window.location.href = update_path[this.props.workflow_data.type].replace(
        "0",
  ...
  getButtons() {
    ...
    if (
      this.props.type == "projectmenu" ||
      this.props.type == "gridmenu" ||
      this.props.type == "exploremenu"
    ) {
      if (this.props.workflow_data.is_owned) {
        buttons.push(
          <div
            class="workflow-delete-button hover-shade"
            onClick={evt => {
              if (
                window.confirm(
                  gettext(
                    "Are you sure you want to delete this? All contents will be deleted, and this action cannot be undone."
                  )
                )
              ) {
                deleteSelf(
      ...
      if (this.props.duplicate) {
        ...
        if (this.props.duplicate == "copy") {
          ...
          buttons.push(
            <div
              class="workflow-duplicate-button hover-shade"
              onClick={evt => {
            ...
            <div
              ...
              onClick={evt => {
                ...
                if (
                  ...
                  this.props.workflow_data.is_strategy
                ) {
                  let loader = new Loader("body");
                  duplicateBaseItem(
                    this.props.workflow_data.id,
                    ...
                    response_data => {
                      try {
                        ...
                        console.log("Couldn't (or didn't need to) update grid");
                      }
                      loader.endLoad();
                    }
                  );
                      ...
                      if (response_data.parentID != null) {
                        ...
                        duplicateBaseItem(
                          ...
                          duplication_response_data => {
                            try {
                              this.props.dispatch(
                                gridMenuItemAdded(duplication_response_data)
                              );
                            } catch (err) {
                              console.log(
                                "Couldn't (or didn't need to) update grid"
                              );
                            }
                            loader.endLoad();
                          }
    ...
    if (this.props.previewAction) {
      buttons.push(
        <div
          class="workflow-view-button hover-shade"
          onClick={evt => {
    ...
    return <div class="workflow-buttons">{buttons}</div>;
  }
}
...
class WorkflowGridMenuUnconnected extends React.Component {
  render() {
    ...
    for (var prop in this.props.data_package) {
      tab_li.push(
        <li>
          <a href={"#tabs-" + i}>{this.props.data_package[prop].title}</a>
        </li>
      );
      tabs.push(
        <MenuTab
...
class ProjectMenuUnconnected extends React.Component {
  ...
  render() {
    ...
    if (!read_only)
      share = (
        ...
        >
          <img src={iconpath + "add_person.svg"} />
          <div>{gettext("Sharing")}</div>
        </div>
      );
    ...
    return (
      <div class="project-menu">
        <div class="project-header">
          ...
          <WorkflowForMenu
            workflow_data={this.state}
            selectAction={this.openEdit.bind(this)}
          />
          <p>
            Disciplines:{" "}
            {this.state.all_disciplines
              .filter(
                discipline => this.state.disciplines.indexOf(discipline.id) >= 0
              )
  ...
  getExportButton() {
    ...
    let export_button = (
      <div
        id="export-button"
        class="floatbardiv hover-shade"
        onClick={() => $(this.exportDropDown.current).toggleClass("activate")}
      >
        <img src={iconpath + "download.svg"} />
    ...
    return reactDom.createPortal(export_button, $("#floatbar")[0]);
  }
  ...
  clickExport(export_type, evt) {
    evt.preventDefault();
    getExport(this.props.project.id, "project", export_type, () =>
      alert(
        gettext(
...
export class ProjectEditMenu extends React.Component {
  ...
  render() {
    ...
    if (data.all_disciplines) {
      all_disciplines = data.all_disciplines
        .filter(discipline => data.disciplines.indexOf(discipline.id) == -1)
        .map(discipline => (
    ...
    let dict_options = Object.keys(custom_text)
      .filter(key => !data.terminology_dict[key])
      .map(key => <option value={key}>{custom_text[key].singular}</option>);
    let selected_term;
    if (this.state.selected_term)
      selected_term = custom_text[this.state.selected_term];
    let dict_added = data.terminology_dict.map(item => (
      <div class="nomenclature-row">
        <div>
          {(custom_text[item.term] && custom_text[item.term].singular) +
            ": " +
            item.translation +
            "/" +
            item.translation_plural}
        </div>
        <div
      ...
      <div class="message-wrap">
        ...
        <div>
          ...
          <div class="multi-select">
            ...
            <button
              id="remove-discipline"
              onClick={this.removeDiscipline.bind(this)}
            >
              {" "}
              {gettext("Remove")}{" "}
            </button>
          ...
          <div class="multi-select">
            ...
            <button id="add-discipline" onClick={this.addDiscipline.bind(this)}>
              {" "}
              {gettext("Add")}{" "}
            </button>
          </div>
        ...
        <div>
          ...
          <input
            id="project-publish-input"
            disabled={!published_enabled}
            type="checkbox"
            name="published"
          ...
          />
          <label for="published">
            {gettext("Is Published (visible to all users)")}
          </label>
        </div>
        <div>
          ...
          <div class="nomenclature-row">
            <select
              ...
              onChange={this.inputChanged.bind(this, "selected_term")}
            >
              <option value="none">{gettext("Select a term")}</option>
              {dict_options}
            </select>
            <input
              placeholder={gettext("custom value")}
              type="text"
              id="term-singular"
              maxlength="50"
              value={data.termsingular}
              onChange={this.inputChanged.bind(this, "termsingular")}
              disabled={selected_term == null || selected_term.singular == null}
            />
            <input
              placeholder={gettext("pluralization")}
              type="text"
              id="term-plural"
              maxlength="50"
              value={data.termplural}
              onChange={this.inputChanged.bind(this, "termplural")}
              disabled={selected_term == null || selected_term.plural == null}
            />
            <button
              onClick={this.addTerm.bind(this)}
              disabled={this.addTermDisabled(selected_term)}
            >
              {gettext("Add")}
            </button>
          </div>
  ...
  addTerm() {
    ...
    addTerminology(
      this.state.id,
      term,
      translation_singular,
      translation_plural,
      response_data => {
        this.setState({
          terminology_dict: response_data.new_dict,
          selected_term: "none",
          termsingular: "",
          termplural: ""
        });
    ...
    this.setState((state, props) => {
      return {
        disciplines: state.disciplines.filter(
          value => selected.map(val => parseInt(val)).indexOf(value) == -1
        )
      };
    });
  }
  ...
  checkboxChanged(field, evt) {
    if (field == "published") {
      if (
        ...
        !window.confirm(
          gettext(
            "Are you sure you want to unpublish this project, rendering it hidden to all other users?"
          )
        )
      ) {
        return;
  ...
  getActions() {
    ...
    actions.push(<button onClick={closeMessageBox}>cancel</button>);
    return actions;
  }
}
...
export class ExploreMenu extends React.Component {
  ...
  render() {
    let objects = this.props.data_package.map(object => (
      <WorkflowForMenu
        selected={this.state.selected == object.id}
        key={object.id}
        type={"exploremenu"}
        workflow_data={object}
        duplicate={"import"}
        objectType={object.type}
        previewAction={this.selectItem.bind(this, object.id, object.type)}
      />
    ));
    ...
    for (let i = 0; i < this.props.pages.page_count; i++) {
      ...
      if (i + 1 == this.props.pages.current_page)
        button_class += " active-page-button";
      page_buttons.push(
        <button class={button_class} onClick={this.toPage.bind(this, i + 1)}>
          {i + 1}
        </button>
      );
    }
        ...
        <form id="search-parameters" action={explore_path} method="GET">
          <div>
            <div>
              ...
              <label>
                <div>{gettext("Author (Username)") + ":"}</div>
                <input
                  autocomplete="off"
                  class="fillable"
                  id="search-author"
          ...
          <div>
            ...
            <label>
              <div>{gettext("Results Per Page:")}</div>
          ...
          <div class="explore-results">
            {objects.length > 0 && [
              <p>
                ...
                {this.props.pages.results_per_page *
                  (this.props.pages.current_page - 1) +
                  1}
                -
                {this.props.pages.results_per_page *
                  this.props.pages.current_page}{" "}
                ({this.props.pages.total_results} {gettext("total results")})
              </p>,
              <p>
                <button
                  id="prev-page-button"
                  disabled={this.props.pages.current_page == 1}
                ...
                <button
                  id="next-page-button"
                  disabled={
                    this.props.pages.current_page == this.props.pages.page_count
                  }
                  onClick={this.toPage.bind(
                    this,
                    this.props.pages.current_page + 1
            ...
            {objects.length == 0 && (
              <p>
                {gettext(
                  "No results were found. Adjust your search parameters, and make sure you have at least one allowed type selected."
                )}
              </p>
            )}
          </div>
          <div class="explore-preview"></div>
        </div>
      </div>
    );
  }
  ...
  componentDidMount() {
    ...
    url_params.forEach((value, key) => {
      if (key.indexOf("[]") >= 0) {
        $(".fillable[name='" + key + "'][value='" + value + "']").attr(
          "checked",
          true
        );
      } else {
    ...
    switch (type) {
      ...
      case "program":
        $.post(post_paths.get_workflow_data, {
          ...
          if (data.action == "posted") {
            loader.endLoad();
            var workflow_renderer = new renderers.WorkflowRenderer(
              JSON.parse(data.data_package)
            );
      ...
      case "project":
        $.post(post_paths.get_project_data, {
          ...
          if (data.action == "posted") {
            loader.endLoad();
            var project_renderer = new renderers.ProjectRenderer(
              data.data_package,
