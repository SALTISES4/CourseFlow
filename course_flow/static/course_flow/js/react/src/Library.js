...
export class LibraryMenu extends React.Component {
  ...
  getCreate() {
    ...
    if (!this.props.renderer.read_only) create = (
      <div class="hover-shade" id="create-project-button" title={gettext("Create project or strategy")}
           ref={this.createDiv}>
        <span class="material-symbols-rounded filled green">add_circle</span>
        <div id="create-links-project" class="create-dropdown">
          <a id="project-create-library" href={config.create_path.project}
             class="hover-shade">{gettext("New project")}</a>
          <hr/>
          <a id="activity-strategy-create" href={config.create_path.activity_strategy}
             class="hover-shade">{gettext("New activity strategy")}</a>
          <a id="course-strategy-create" href={config.create_path.course_strategy}
             class="hover-shade">{gettext("New course strategy")}</a>
        </div>
      </div>
    )
    return create;
...
export class ExploreMenu extends LibraryMenu {
  ...
  render() {
    return (
      <div class="project-menu">
        <ExploreFilter disciplines={this.props.disciplines} renderer={this.props.renderer}
                       workflows={this.props.renderer.initial_workflows} pages={this.props.renderer.initial_pages}
                       context="library"/>
      </div>
    );
  }
...
export class ProjectMenu extends LibraryMenu {
  ...
  getContent() {
      ...
      <div class="workflow-view-select hide-print">
        {this.getViewButtons().map(
          (item) => {
            ...
            return <a id={"button_" + item.type} class={view_class}
                      onClick={this.changeView.bind(this, item.type)}>{item.name}</a>;
          }
        )}
      </div>
    );
    switch (this.state.view_type) {
      case "overview":
        return_val.push(<LiveProjectViews.LiveProjectOverview renderer={this.props.renderer} role={this.getRole()}
                                                              objectID={this.state.data.id}
                                                              view_type={this.state.view_type}/>);
        break;
      case "students":
        return_val.push(<LiveProjectViews.LiveProjectStudents renderer={this.props.renderer} role={this.getRole()}
                                                              objectID={this.state.data.id}
                                                              view_type={this.state.view_type}/>);
        break;
      case "assignments":
        return_val.push(<LiveProjectViews.LiveProjectAssignments renderer={this.props.renderer} role={this.getRole()}
                                                                 objectID={this.state.data.id}
                                                                 view_type={this.state.view_type}/>);
        break;
      case "completion_table":
        return_val.push(<LiveProjectViews.LiveProjectCompletionTable renderer={this.props.renderer}
                                                                     role={this.getRole()} objectID={this.props.data.id}
                                                                     view_type={this.state.view_type}/>);
        break;
      // case "settings":
      ...
      //     break;
      default:
        return_val.push(<WorkflowFilter renderer={this.props.renderer} workflows={this.state.workflow_data}
                                        updateWorkflow={this.updateWorkflow.bind(this)} context="project"/>);
    }
    return return_val;
  }
  ...
  getOverflowLinks() {
    ...
    if (data.author_id == user_id) {
      if (data.liveproject) {
        liveproject = (
          <a id="live-project" class="hover-shade"
             href={config.update_path.liveproject.replace("0", data.id)}>{gettext("View Classroom")}</a>
        );
      } else {
        liveproject = (
  ...
  getExportButton() {
    ...
    let export_button = (
      <div id="export-button" class="hover-shade"
           onClick={() => renderMessageBox(this.state.data, "export", closeMessageBox)}>
  ...
  getEdit() {
    let edit;
    if (!this.props.renderer.read_only) edit =
      <div class="hover-shade" id="edit-project-button" title={gettext("Edit Project")}
           onClick={this.openEditMenu.bind(this)}><span class="material-symbols-rounded filled">edit</span></div>
    return edit;
  }
  ...
  openEditMenu() {
    ...
    }, "project_edit_menu", this.updateFunction.bind(this));
  }
  ...
  getCreate() {
    ...
    if (!this.props.renderer.read_only) create = (
      <div class="hover-shade" id="create-project-button" title={gettext("Create workflow")} ref={this.createDiv}>
        ...
        <div id="create-links-project" class="create-dropdown">
          <a id="activity-create-project" href={create_path_this_project.activity}
             class="hover-shade">{gettext("New activity")}</a>
          <a id="course-create-project" href={create_path_this_project.course}
             class="hover-shade">{gettext("New course")}</a>
          <a id="program-create-project" href={create_path_this_project.program}
             class="hover-shade">{gettext("New program")}</a>
        </div>
      </div>
    )
    return create;
  ...
  getShare() {
    let share;
    if (!this.props.renderer.read_only) share =
      <div class="hover-shade" id="share-button" title={gettext("Sharing")} onClick={this.openShareMenu.bind(this)}>
        <span class="material-symbols-rounded filled">person_add</span></div>
    return share;
  }
  ...
  render() {
    ...
    if (!this.state.workflows) workflows = this.defaultRender();
      ...
      workflows = workflows.map(workflow =>
        <WorkflowForMenu renderer={this.props.renderer} key={workflow.type + workflow.id} workflow_data={workflow}
                         context={this.props.context} updateWorkflow={this.props.updateWorkflow}/>
      );
    }
    let search_results = this.state.search_results.map(workflow =>
      <WorkflowForMenuCondensed key={workflow.type + workflow.id} workflow_data={workflow}
...
export class ExploreFilter extends WorkflowFilter {
      ...
      [
        <div class="workflow-filter-top">
          <div class="flex-middle">
            ...
            <button class="primary-button" disabled={this.state.has_searched}
                    onClick={this.doSearch.bind(this)}>{gettext("Search")}</button>
          </div>
          <div class="workflow-filter-sort">
            {this.getFromSaltise()}
            {this.getContentRich()}
      ...
      if (this.state.pages.current_page < this.state.pages.page_count - 2) {
        ...
        page_buttons.push(
          <button class="page-button"
                  onClick={this.toPage.bind(this, this.state.pages.page_count)}>{this.state.pages.page_count}</button>
        );
      }

      page_buttons.push(
        <button id="next-page-button" disabled={(this.state.pages.current_page == this.state.pages.page_count)}
                onClick={
                  this.toPage.bind(this, this.state.pages.current_page + 1)
  ...
  getFilter() {
    return (
      <div id="workflow-filter" ref={this.filterDOM} class="hover-shade">
        ...
        <div attr_number={this.state.active_filters.length}
             class="dropdown-number-indicator">{this.state.active_filters.length}</div>
      </div>
    );
  }
  ...
  getDisciplines() {
    ...
    return (
      <div id="workflow-disciplines" ref={this.disciplineDOM} class="hover-shade">
        ...
        <div attr_number={this.state.active_disciplines.length}
             class="dropdown-number-indicator">{this.state.active_disciplines.length}</div>
      </div>
    );
  }
  ...
  getFromSaltise() {
    let component = this;
    return (
      <div title={gettext("Restrict results to content provided by SALTISE")} id="content-rich" class="hover-shade"
           onClick={
             () => {
...
export class WorkflowForMenu extends React.Component {
  ...
  render() {
    ...
    return (
      <div ref={this.maindiv} class={css_class} onClick={this.clickAction.bind(this)} onMouseDown={(evt) => {
        evt.preventDefault()
      }}>
  ...
  getButtons() {
    ...
    if (this.props.workflow_data.type == "project" && this.props.workflow_data.has_liveproject && this.props.workflow_data.object_permission.role_type != Constants.role_keys["none"]) workflows.push(
      <div class="workflow-created workflow-live-classroom">
        <span class="material-symbols-rounded small-inline"
              title={gettext("Live Classroom")}>group</span>{" " + gettext("Live Classroom")}
      </div>
    );
    if (this.props.workflow_data.is_linked) workflows.push(
      <div class="workflow-created linked-workflow-warning"
        ...
        <span class="material-symbols-rounded red filled small-inline">error</span>{" " + gettext("Already in use")}
      </div>
    )
    return (
  ...
  getVisible() {
    ...
    if (this.props.renderer && !this.props.renderer.read_only && this.props.renderer.user_role === Constants.role_keys.teacher && this.props.workflow_data.type !== "project" && this.props.workflow_data.type !== "liveproject" && this.props.renderer && this.props.renderer.user_role === Constants.role_keys.teacher) return (
      <div class="permission-select" onClick={(evt) => evt.stopPropagation()}
           onMouseDown={(evt) => evt.stopPropagation()}>
        <select value={this.props.workflow_data.is_visible}
