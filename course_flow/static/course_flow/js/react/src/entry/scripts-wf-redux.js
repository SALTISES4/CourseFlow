...
export class WorkflowRenderer {
    ...
    if (!this.always_static) {
      ...
      updateSocket.onclose = function(e) {
        ...
        if (!renderer.silent_connect_fail && !renderer.has_disconnected) {
          alert(gettext('Unable to establish connection to the server, or connection has been lost.'));
        }
  ...
  getDataForChildWorkflow() {
    ...
    this.getWorkflowChildData(
      ...
      (response) => {
        this.store.dispatch(
          Reducers.refreshStoreData(response.data_package)
        );
  ...
  attempt_reconnect() {
    let renderer = this;
    setTimeout(() => { renderer.connect() }, 30000);
  }
  ...
  change_field(id, object_type, field, value) {
    ...
    json[field] = value;
    this.store.dispatch(
      Reducers.changeField(id, object_type, json)
    );
