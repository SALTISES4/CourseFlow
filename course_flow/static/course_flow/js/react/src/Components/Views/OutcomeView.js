...
class OutcomeView extends EditableComponentWithSorting {
  ...
  render() {
    ...
    if (this.state.show_horizontal_links) outcomehorizontallinks = (
      <div className={"outcome-node-container"} onMouseLeave={() => {
    ...
    if (this.props.show_horizontal && data.outcome_horizontal_links_unique.length > 0) {
      side_actions.push(
        <div className="outcome-node-indicator">
          <div className={"outcome-node-indicator-number"} onMouseEnter={() => {
            this.setState({show_horizontal_links: true});
          }}>{data.outcome_horizontal_links_unique.length}</div>
    ...
    return (
      <div style={style}
        ...
        {data.depth < 2 &&
          <ol class={"children-block children-block-" + this.props.data.depth}
              id={this.props.objectID + "-children-block"} ref={this.children_block}>
            {children}
          </ol>
        }
        {(!this.props.renderer.read_only && data.depth < 2) &&
          <div class="outcome-create-child" onClick={this.insertChild.bind(this, data)}>{gettext("+ Add New")}</div>
        }
  ...
  render() {
    ...
    return (
      <div
        ...
        <input class="outcome-toggle-checkbox" type="checkbox" title="Toggle highlighting"
...
export class SimpleOutcomeViewUnconnected extends EditableComponentWithComments {
  ...
  render() {
    ...
    return (
      <div className={css_class} style={this.get_border_style()}
        ...
        {data.depth < 2 && data.child_outcome_links.length > 0 &&
          <div
            ...
            <div className="outcome-drop-text">
              {droptext}
            </div>
          </div>
        }
...
class OutcomeHorizontalLinkViewUnconnected extends Component {
  ...
  render() {
    ...
    return (
      <div class={"outcome-node outcome-" + data.id} id={data.id} ref={this.maindiv}>
        ...
        <SimpleOutcomeView renderer={this.props.renderer} checkHidden={this.checkHidden.bind(this)}
                           objectID={data.parent_outcome} parentID={this.props.parentID} throughParentID={data.id}/>
      </div>
    );
  }
  ...
  addDeleteSelf(data) {
    let icon = "close.svg";
    return (
      <ActionButton button_icon={icon} button_class="delete-self-button" titletext={gettext("Delete")}
