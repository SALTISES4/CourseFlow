...
export async function expectPasswordResetPagePrimaryLayoutPerFrPwd001(
  page: Page,
): Promise<void> {
  await waitForPasswordResetPageLoaded(page);
  ...
  await expect(currentPasswordField(page)).toHaveAttribute('required', '');
  ...
  await expect(newPasswordField(page)).toHaveAttribute('required', '');
  await expect(confirmNewPasswordField(page)).toBeVisible();
  await expect(confirmNewPasswordField(page)).toBeEditable();
  await expect(confirmNewPasswordField(page)).toHaveAttribute(
    'type',
    'password',
  );
  await expect(confirmNewPasswordField(page)).toHaveAttribute('required', '');
  await expect(passwordNewPasswordGuidelines(page)).toBeVisible();
  await expect(passwordNewPasswordGuidelines(page)).toHaveText(
    PASSWORD_NEW_PASSWORD_GUIDELINES,
  );
...
export async function expectPasswordResetFormInitialStatePerFrPwd002(
  page: Page,
): Promise<void> {
  ...
  for (const message of Object.values(PASSWORD_RESET_VALIDATION_MESSAGES)) {
    await expect(
      passwordResetFieldValidationMessage(page, message),
    ).toHaveCount(0);
  }
}
