...
> extends React.Component<P, S> {
  ...
  EditForm = ({ data, noDelete }) => {
    ...
    const titleLength = type === 'outcome' ? 500 : 100
    ...
    if (this.props.objectSets && ['node', 'outcome'].indexOf(type) >= 0) {
      ...
      const allowedSets = this.props.objectSets.filter(
        (set) => set.term == termType
      )

      if (allowedSets.length >= 0) {
        const disableSets = data.depth || readOnly ? true : false
        const setOptions = allowedSets.map((set, index) => (
          <div key={`${set.id}_${index}`}>
            <input
              disabled={disableSets}
              type="checkbox"
              name={set.id}
        ...
        sets = [<h4 key="setTitle">{_t('Sets')}</h4>, setOptions]
      }
    }
