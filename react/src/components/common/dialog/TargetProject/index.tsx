...
function TargetProjectDialog({id,actionFunction}:any) {
  ...
  const getContent = ()=>{
    if(show){
      if( project_data == null) getTargetProjectMenuQuery(id,setProjectData)
      else return <TargetProjectDialogContents data={project_data} onDialogClose={onDialogClose}/>
    }else return null
  }
...
function TargetProjectDialogContents({data,onDialogClose}:TargetProjectDialogContentsType){
