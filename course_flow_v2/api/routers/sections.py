...
@workflow_collection_router.get(
    "/{workflow_uuid}/sections", response=SectionListOut, auth=BearerAuth()
)
def list_workflow_sections(request, workflow_uuid: UUID):
    current_user = get_current_user(request)
    _ensure_workflow_owner(workflow_uuid, current_user)
    rows = get_section_service().list_for_workflow_uuid(workflow_uuid)
...
@workflow_collection_router.post(
    "/{workflow_uuid}/sections", response=SectionOut, auth=BearerAuth()
)
def create_workflow_section(
    request, workflow_uuid: UUID, payload: WorkflowSectionCreateIn
):
    current_user = get_current_user(request)
    _ensure_workflow_owner(workflow_uuid, current_user)
...
@resource_router.post("", response=SectionOut, auth=BearerAuth())
...
@resource_router.get("/{uuid}", response=SectionOutResp, auth=BearerAuth())
...
@resource_router.patch("/{uuid}", response=SectionOutResp, auth=BearerAuth())
