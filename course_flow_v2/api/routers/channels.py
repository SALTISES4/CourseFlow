...
def _ensure_workflow_owner(workflow_uuid: UUID, current_user) -> None:
    dto = get_workflow_service().get_by_uuid(workflow_uuid)
    if dto is None:
...
@workflow_collection_router.get(
    "/{workflow_uuid}/channels", response=ChannelListOut, auth=BearerAuth()
)
def list_workflow_channels(request, workflow_uuid: UUID):
    current_user = get_current_user(request)
    _ensure_workflow_owner(workflow_uuid, current_user)
    rows = get_channel_service().list_for_workflow_uuid(workflow_uuid)
...
@workflow_collection_router.post(
    "/{workflow_uuid}/channels", response=ChannelOut, auth=BearerAuth()
)
def create_workflow_channel(
    request, workflow_uuid: UUID, payload: WorkflowChannelCreateIn
):
    current_user = get_current_user(request)
    _ensure_workflow_owner(workflow_uuid, current_user)
...
@resource_router.post("", response=ChannelOut, auth=BearerAuth())
...
@resource_router.get("/{uuid}", response=ChannelOutResp, auth=BearerAuth())
...
@resource_router.patch("/{uuid}", response=ChannelOutResp, auth=BearerAuth())
...
@resource_router.delete("/{uuid}", auth=BearerAuth())
